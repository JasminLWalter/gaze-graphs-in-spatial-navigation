...
% Purpose: 
% Step 1 of the VR eye-tracking pre-processing pipeline. Starting from flattened logs,
% harmonizes collider names (change list; graffiti → TaskBuilding), replaces body hits
% with second hits when applicable, labels non-house colliders as 'NH' (and uses second hits
% if valid), masks low-validity samples as 'noData', writes a renamed/reduced CSV, and
% condenses consecutive samples on the same collider into clusters (condensedColliders_WB.mat).
%
% Usage:
...
% - Mapping CSVs: additional_Files/building_collider_list.csv; additional_Files/list_collider_changes.csv
% - Run the script in MATLAB.
%
% Inputs:
% - Flattened ET CSVs in the current folder (per participant/session)
% --> raw csvs from the OSF repository https://osf.io/qcn67
% - Mapping CSVs as listed above
%
% Outputs:
% - Per ET file (savepathNewData):
%   - <ID>Session<S>ET<E>_rawData_renamedColliders.csv
% - Per ET file (savepathCondensedData):
%   - <ID>Session<S>ET<E>_condensedColliders_WB.mat (variable: condensedData)
% - Overviews:
%   - savepathNewData: overviewRenamedGraffiti.mat; overviewRemainingColliders.mat; overviewAllColliders.mat; overviewMissingData.csv
%   - savepathCondensedData: overviewMissingData.csv
% - Missing file report (if any): missingFiles.csv in both save paths
%
...
savepathNewData = '...\Pre-processsing_pipeline\rawData_with_renamedColliders\';
savepathCondensedData = '...\Pre-processsing_pipeline\condensedColliders\';

cd '...\Westbrook_eye-motion-tracking_rawCSVs\'
...
colliderList = readtable('...\Github\gaze-graphs-in-spatial-navigation\additional_Files\building_collider_list.csv');

changedColliders = readtable('...\Github\gaze-graphs-in-spatial-navigation\additional_Files\list_collider_changes.csv');
