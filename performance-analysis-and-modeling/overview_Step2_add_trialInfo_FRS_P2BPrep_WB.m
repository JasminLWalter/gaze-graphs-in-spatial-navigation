%% ------------------ overview_Step2_add_trialInfo_FRS_P2BPrep_WB.m-----------------------

% --------------------script written by Jasmin L. Walter-------------------
% -----------------------jawalter@uni-osnabrueck.de------------------------

% Purpose: Merges FRS questionnaire summaries into PTB trial data, assigns route IDs to unique
%          StartBuilding–TargetBuilding pairs, determines trial order (initial vs. repeat),
%          computes repetition error metrics, and derives trial sequence indices.
%
% Usage:
% - Adjust: savepath and working directory (cd).
% - Ensure the required CSVs exist in the working directory.
% - Run the script in MATLAB.
%
% Inputs:
% - selectedData_P2B_control.csv (columns incl.: SubjectID, TimeStampBegin, TrialDuration,
%   StartBuildingName, TargetBuildingName, RecalculatedAngle, DistancePart2TargetBuilding)
% - Overview_FRS_Data.csv (columns incl.: Participant, Mean_egocentric_global, Mean_survey, Mean_cardinal)
%
% Outputs (to savepath):
% - uniqueTrials_routeID.mat/.csv (map of unique Start–Target pairs to RouteID)
% - overviewTable_P2B_Prep_complete.mat/.csv (with FRS means, RouteID, TrialOrder, TrialTime,
%   TrialSequence, TrialSequence_SameStart14/7)
% - overviewTable_P2B_Prep_complete_withoutReps.mat/.csv (repetition-averaged, reps removed)
% - trialRepDiffAbs.mat; trialRepDiffMean.mat; trialRepDiffStd.mat
% - overviewTrialRepDiff_absoluteError.mat/.csv (per-participant absolute error by RouteID)
% - overviewRepetitionData.mat/.csv (per-trial Abs/Mean/Std error)
%
...
%% load overview
overviewTableP2BPrep3 = readtable('selectedData_P2B_control.csv');
